...
**Rules**: append-only; one commit per event; commit message encodes `event/actor/ts`; **never amended or rebased** (corrections = new commits, Constitution IV / FR-012). Unbounded (FR-012a) — `git gc` only, no rotation. Repo is separate from the netgeniusclaw source repo.
