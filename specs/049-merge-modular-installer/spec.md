...
1. **Given** the full list of MCP servers currently registered in NetGeniusClaw's reference configuration, **When** the coverage check runs against the merged installer's catalog, **Then** every one of those servers is represented by a catalog entry (either one-to-one, or as part of a deliberately documented group, such as the existing consolidation of the 15 Check Point servers into one entry).
...
- **FR-001**: System MUST provide a repeatable way to verify that every MCP server currently registered in NetGeniusClaw's reference configuration has a corresponding entry in the merged installer's component catalog.
