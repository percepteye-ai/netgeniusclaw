...
**Created**: 2026-06-25
**Status**: Draft
**Input**: User description: "Add bidirectional phone call capabilities to NetGeniusClaw using Twilio for emergency alerts, situation updates, daily check-ins, on-demand calls, AND the ability to call NetGeniusClaw for voice conversations. Strict guardrails required."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Emergency Alert Calls (Priority: P1)

As John, I want NetGeniusClaw to call me immediately when critical network events occur so that I can respond to emergencies even when not actively monitoring screens.
...
**Independent Test**: Can be tested by simulating a P1 PagerDuty incident and verifying NetGeniusClaw initiates a phone call with the incident summary.

**Acceptance Scenarios**:

1. **Given** a P1 incident is triggered in PagerDuty, **When** NetGeniusClaw processes the incident, **Then** it immediately calls John's whitelisted phone number with a voice summary of the incident
2. **Given** critical device failure is detected (core router, firewall, WAN link), **When** NetGeniusClaw classifies it as emergency-level, **Then** it initiates a phone call within 60 seconds
3. **Given** an emergency call is in progress, **When** the call connects, **Then** NetGeniusClaw speaks a clear, concise summary using text-to-speech (no sensitive data like IPs or credentials)
4. **Given** the call fails to connect (voicemail, no answer), **When** 30 seconds elapse, **Then** NetGeniusClaw retries once and logs the attempt

---

### User Story 2 - On-Demand Status Calls (Priority: P2)

As John, I want to explicitly request NetGeniusClaw to call me with a status update so that I can get voice briefings while away from my desk.
...
**Independent Test**: Can be tested by issuing a command like "Call me with the current network status" and verifying NetGeniusClaw initiates a call with the requested information.

**Acceptance Scenarios**:

1. **Given** John sends "call me with network status" via Slack or CLI, **When** NetGeniusClaw processes the request, **Then** it calls John's number and speaks current network health summary
2. **Given** John requests a call about a specific topic (e.g., "call me about the CML lab status"), **When** NetGeniusClaw processes the request, **Then** the call content focuses on that specific topic
3. **Given** John is already on an active NetGeniusClaw call, **When** he requests another call, **Then** the system queues the request and notifies him

---

### User Story 3 - Situation Update Calls (Priority: P2)

As John, I want NetGeniusClaw to call me with updates during ongoing incidents when I request it so that I can stay informed while handling other responsibilities.
...
1. **Given** an active incident is being tracked, **When** John says "call me every 30 minutes with updates", **Then** NetGeniusClaw schedules periodic calls until the incident is resolved or John cancels
2. **Given** a scheduled update call is due, **When** there are no status changes since last call, **Then** NetGeniusClaw skips the call and notifies via text instead (to avoid unnecessary calls)
3. **Given** significant progress occurs on the incident, **When** the change is critical (e.g., resolved, escalated), **Then** NetGeniusClaw calls immediately regardless of schedule

---

### User Story 4 - Daily Check-in Calls (Priority: P3)

As John, I want to optionally enable daily phone briefings from NetGeniusClaw so that I can start my day with a voice summary of network health.
...
1. **Given** daily briefings are enabled with a scheduled time (e.g., 8:00 AM), **When** the scheduled time arrives, **Then** NetGeniusClaw calls with a summary of overnight events and current health status
2. **Given** no significant events occurred overnight, **When** the briefing call connects, **Then** NetGeniusClaw delivers a brief "all clear" message (under 30 seconds)
...
As John, I want to call a dedicated phone number and have a voice conversation with NetGeniusClaw so that I can interact with my network assistant hands-free while driving or away from screens.
...
**Independent Test**: Can be tested by calling the NetGeniusClaw phone number and having a spoken conversation about network status.

**Acceptance Scenarios**:

1. **Given** John calls the dedicated NetGeniusClaw phone number, **When** the call connects, **Then** NetGeniusClaw greets him and listens for voice commands
2. **Given** John speaks a question like "What's the status of my CML lab?", **When** NetGeniusClaw processes the speech, **Then** it responds with the relevant information via voice
3. **Given** John asks NetGeniusClaw to perform an action (e.g., "Run a health check on the core routers"), **When** the action requires approval, **Then** NetGeniusClaw asks for verbal confirmation before proceeding
4. **Given** John says "goodbye" or hangs up, **When** the call ends, **Then** NetGeniusClaw logs the conversation summary
5. **Given** an unknown caller dials the NetGeniusClaw number, **When** caller ID doesn't match the whitelist, **Then** the call is rejected with a polite message
...
**Independent Test**: Can be tested by calling NetGeniusClaw and issuing a voice command that triggers a network action.

**Acceptance Scenarios**:

1. **Given** John is on a call with NetGeniusClaw, **When** he says "Show me BGP neighbors on router-1", **Then** NetGeniusClaw speaks the BGP neighbor summary
2. **Given** John requests a potentially impactful action, **When** the command involves configuration changes, **Then** NetGeniusClaw requires verbal confirmation ("Say 'confirm' to proceed")
3. **Given** John asks for information that would contain sensitive data, **When** responding, **Then** NetGeniusClaw sanitizes the response (e.g., "Interface on the core router is down" instead of revealing the IP)
...
- What happens if NetGeniusClaw attempts to call outside quiet hours? Calls are blocked during configured quiet hours (e.g., 10 PM - 7 AM) except for P1 emergencies.
...
- What if speech recognition fails to understand John? NetGeniusClaw asks for clarification up to 2 times, then suggests using Slack instead.
- What if someone other than John calls the NetGeniusClaw number? Caller ID is checked against whitelist; unauthorized callers hear a rejection message and are logged.
...
- **FR-013**: System MUST provide a dedicated phone number that John can call to reach NetGeniusClaw
