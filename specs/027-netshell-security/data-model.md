...
DefenseClaw manages its own data model internally. NetGeniusClaw's integration is minimal - we only configure the security mode in `openclaw.json`. DefenseClaw handles:
...
The security configuration section in NetGeniusClaw's main config file.
...
|------|----------|--------|-------|
| Security Mode | `~/.openclaw/config/openclaw.json` | JSON | NetGeniusClaw |
