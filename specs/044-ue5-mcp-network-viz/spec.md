...
**Created**: 2026-06-28
**Status**: Draft
**Input**: User description: "Connect NetGeniusClaw to the Unreal Engine 5.8 MCP server to enable 3D network topology visualization. Users should be able to ask 'render my network in UE5' and have NetGeniusClaw collect topology data from existing MCP servers, translate that into UE5 actor spawning commands, and build a live 3D scene with real-time updates."
...
1. **Given** NetGeniusClaw has access to a network data source with topology information, **When** the user says "render my network in UE5", **Then** a 3D scene is generated in Unreal Engine with one actor per network device and visual connections representing links.
2. **Given** the UE5 MCP server is running at localhost:8000/mcp, **When** NetGeniusClaw sends actor spawning commands, **Then** devices appear in the scene within 10 seconds of command completion.
...
- **FR-002**: System MUST collect network topology data from at least one connected NetGeniusClaw data source (pyATS, SuzieQ, GNS3, CML, or similar).
...
- UE5.8 with the MCP plugin is installed and running on the local machine before NetGeniusClaw commands are issued.
- The UE5 MCP server is accessible at localhost:8000/mcp (default configuration) or user has configured alternate endpoint.
- At least one NetGeniusClaw network data source (pyATS, SuzieQ, GNS3, CML) is connected and contains topology information.
