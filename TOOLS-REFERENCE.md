...
`computer-use` is OpenClaw's own ClawHub skill (not an MCP server — no `config/openclaw.json` entry). It provisions a virtual X11 desktop (Xvfb + XFCE) on this NetGeniusClaw host and exposes 17 actions as individual bash scripts (`screenshot.sh`, `click.sh`, `type_text.sh`, `key.sh`, `scroll.sh`, `drag.sh`, `zoom.sh`, `wait.sh`, `cursor_position.sh`, `mouse_move.sh`, `mouse_down.sh`, `mouse_up.sh`, `hold_key.sh`), invoked directly against `DISPLAY=:99`.
