...
The existing `mcp-servers/azure-network-mcp/utils/rate_limiter.py` is an Azure SDK exception translator — not a real rate limiter. There is no token-bucket pattern in NetGeniusClaw to copy, so this PR introduces one. It is the simplest design that works:
...
## Comparison to existing FastMCP REST servers in NetGeniusClaw
...
NetGeniusClaw currently has two lab-mode env vars in use:
