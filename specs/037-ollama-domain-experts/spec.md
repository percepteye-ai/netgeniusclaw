# Feature Specification: Ollama Domain Expert Delegation MCP (NetGeniusClaw Demo Edition)
...
**Input**: User description: "Point NetGeniusClaw's Frontier model to an Ollama MCP that delegates domain-specific tasks to local LLMs — OSPF expert, BGP expert, RFC design validator, Nautobot expert — specifically tuned to handle the failure modes observed during previous NetGeniusClaw demo runs."

## Problem Statement: Why Single LLMs Fail the Demo

The NetGeniusClaw SP core demo (6-node FRR lab, Nautobot SOT, config push, validate) has been run dozens of times with various LLMs. Consistent failure modes emerge:
...
3. **JSON-in-JSON parameter encoding** — `nautobot_run_job(data='{"deployment_name": "NetGeniusClaw Demo"}')` confuses models that try to pass dicts or double-encode
...
- **FR-011**: System MUST be usable from both OpenClaw (NetGeniusClaw) and Kiro IDE without modification.
