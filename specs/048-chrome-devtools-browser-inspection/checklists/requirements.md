...
- The feature is inherently about integrating a named upstream tool (`chrome-devtools-mcp`) and a named authentication mechanism (`--userDataDir`); these are named because they are the subject of the feature itself (consistent with how other NetGeniusClaw specs name the specific community MCP/vendor SDK they integrate), not incidental implementation choices leaking into requirements or success criteria.
