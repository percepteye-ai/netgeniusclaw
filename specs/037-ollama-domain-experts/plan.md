...
**Target Platform**: Linux (same as all NetGeniusClaw MCP servers)

**Project Type**: MCP server (stdio transport)

**Performance Goals**: <30s response for 7B model config generation, <120s for 35B models

**Constraints**: Must work with Ollama HTTP API (no custom protocols), must fit existing NetGeniusClaw MCP patterns
