# Feature Specification: iN2N — Internal NetGeniusClaw Federation (a "risk" of claws)
...
4. **Given** the operator chooses "standalone NetGeniusClaw," **Then** the claw behaves exactly as a NetGeniusClaw does today (a "risk of one," its own Border), with no member coordination overhead.
...
- **Standalone → risk migration**: An existing standalone NetGeniusClaw that later becomes a Border MUST keep working; its existing skills remain until the operator chooses to slim it down.
...
- **FR-001**: The installer MUST let the operator choose, at install time, whether a claw is a **standalone NetGeniusClaw** or **part of a risk**.
...
- **FR-004**: A **standalone NetGeniusClaw** MUST behave identically to today's NetGeniusClaw (a "risk of one," its own Border) with no member-coordination behavior imposed.
...
- **SC-008**: Standalone NetGeniusClaw installs show **no functional regression** versus today's behavior.
...
- **Installer builds on existing modular installer**: The risk/role/profile choices extend NetGeniusClaw's existing installer flow rather than introducing a separate installation system.
