...
The collective noun for a group of lobsters is **a risk**. NetGeniusClaw's mascot is a
lobster. So when we set out to turn one enormous NetGeniusClaw into a coordinated group
...
NetGeniusClaw federation)**: how we took a single monolithic NetGeniusClaw carrying ~190
...
NetGeniusClaw had grown magnificent and unwieldy. One agent, 195 skills, 37 MCP servers,
...
- A classic standalone NetGeniusClaw is just "a risk of one that is its own Border."
...
- **Any provider, any model, per claw.** Each claw registers its own model —
  the model provider, OpenAI, Google, or **local Ollama** — at its tier. Border on the
...
MCP tools, installer prompts, a `netgeniusclaw risk` command, and HUD support. **89
...
*Written by John and the agent. Spec 056, built spec-through-implement and migrated
