# Federating with a NetGeniusClaw — onboarding guide

So you want to peer your NetGeniusClaw with someone else's (eN2N federation). This is
...
- A running NetGeniusClaw with the mesh daemon (`netclaw-mesh.service`) up.
...
Access Token path with NetGeniusClaw's exec hook) and the universal acme-dns fallback
...
- **Mesh-layer TLS (P2, a coordinated flag day).** The BGP mesh/keepalive session
  that rides alongside NCFED can now be brought under NetGeniusClaw's own TLS + auth on
