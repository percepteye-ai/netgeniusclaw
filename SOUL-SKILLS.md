# NetGeniusClaw Skill Procedures Reference
...
4. Cross-reference with other NetGeniusClaw sources (CML labs, SuzieQ state)
...
conn.log with 3 rows instead of 2. NetGeniusClaw's own capture skills produce such captures, so the
...
**NetGeniusClaw's own stores are unreachable by construction.** Datasets are materialised, then
...
| `gait-session-tracking` | GAIT session lifecycle management - branch creation, turn recording, audit logging for every NetGeniusClaw operation. Use when starting a new NetGeniusClaw session, reco... | `workspace/skills/gait-session-tracking/SKILL.md` |
...
| `ipfabric` | > Developed in collaboration with **Daren Fulwell** (Field CTO, IP Fabric) and **John Capobianco** (Creator, NetGeniusClaw), representing nearly a decade of profes... | `workspace/skills/ipfabric/SKILL.md` |
...
| `memory` | The Memory skill enables NetGeniusClaw to remember information about your network across sessions. Instead of re-explaining your topology, device names, and past i... | `workspace/skills/memory/SKILL.md` |
...
| `threejs-network-viz` | Renders network topologies as interactive, fully-labeled 3D scenes directly in a web browser, using Three.js. Unlike NetGeniusClaw's UE5 (`ue5-network-viz`) and Bl... | `workspace/skills/threejs-network-viz/SKILL.md` |
| `token-tracker` | Track and display token consumption and cost for every NetGeniusClaw interaction. | `workspace/skills/token-tracker/SKILL.md` |
...
| `twilio-inbound-voice` | Enable John to call NetGeniusClaw's Twilio number and have a bidirectional voice conversation for status queries and network commands. | `workspace/skills/twilio-inbound-voice/SKILL.md` |
...
| `twitter-heartbeat` | The twitter-heartbeat skill enables NetGeniusClaw to: | `workspace/skills/twitter-heartbeat/SKILL.md` |
...
| `twitter-share` | The twitter-share skill enables users to ask NetGeniusClaw to tweet specific content. All tweets go through content guardrails and require explicit human approval ... | `workspace/skills/twitter-share/SKILL.md` |
