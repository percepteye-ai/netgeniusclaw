...
```json
{
  ...
  "location": [0, 0, 100],
  "scale": [50, 50, 50],
  "tags": ["netgeniusclaw", "device", "router"]
}
```
...
**Usage**: Query existing NetGeniusClaw actors for incremental updates.
...
**Usage**: Clear all NetGeniusClaw actors before full re-render.
...
| "Clear the scene" | `clear_actors_with_tag(netgeniusclaw)` |
...
| "What devices are rendered?" | `get_all_actors_with_tag(netgeniusclaw)` → filter devices |
