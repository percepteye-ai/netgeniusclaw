...
## Teach NetGeniusClaw something (three ways)

```text
# 1. Slack: drag a PDF into the NetGeniusClaw channel
you>  (attach wlc-9800-upgrade-guide.pdf) learn this — it's a vendor guide
netgeniusclaw> Learned "WLC 9800 Upgrade Guide" (vendor, 84 pages, 217 chunks, documents).
...
netgeniusclaw> That page links to 6 same-domain pages. Ingest just the page, or all 7?
you>  all 7
netgeniusclaw> Ingested 7 pages into documents. …

# 3. HUD: open the Knowledge panel → drag file → watch parsing → chunking → embedding → done
```

## Ask (agentic retrieval — NetGeniusClaw decides)

```text
you>  upgrade the lab WLC following our customer standards
netgeniusclaw> (decomposes, searches vendor guide + customer standard, critiques, refines ≤3 rounds/sub-query)
...
netgeniusclaw> Scope: PE1, PE2 — "show ip bgp" via pyATS, into snapshot_core-bgp_<timestamp>. Confirm?
you>  yes
netgeniusclaw> Stored 48 chunks. Redactions: password: 2, snmp_community: 1, bgp_auth_key: 0 …
# a month later
netgeniusclaw> From snapshot core-bgp — captured 2026-07-16 14:02 UTC — 31 days ago (live state available via MCP): …
```
...
- **RAG ≠ Memory**: `~/.openclaw/rag/` (user-uploaded knowledge) is fully separate from `~/.openclaw/memory/` (NetGeniusClaw's own experience). Neither writes into the other.
...
- **Every RAG claim is cited**; anything NetGeniusClaw can't cite, it won't attribute to the knowledge base.
