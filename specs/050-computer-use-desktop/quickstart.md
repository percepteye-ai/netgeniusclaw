...
1. A Linux host running NetGeniusClaw (the upstream `computer-use` skill targets headless Linux servers — not macOS; see research.md R1 on why `codex-computer-use` doesn't apply here)
...
3. `openclaw` CLI available (already true on any NetGeniusClaw install)
