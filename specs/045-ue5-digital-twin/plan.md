...
**Language/Version**: Python 3.10+ (matches the existing `ue5-network-viz` skill and the rest of NetGeniusClaw)
**Primary Dependencies**: httpx (existing UE5 MCP HTTP/JSON-RPC client, `ue5_mcp_client.py`), no new third-party packages required
**Storage**: N/A — all new state (sticky alert flags, live-mode status, session history buffer, manual zoom groupings) is in-memory for the lifetime of the running skill process; nothing persists across a NetGeniusClaw restart
...
**Target Platform**: WSL2/Linux (NetGeniusClaw) + Windows (UE5 Editor GUI) — unchanged from 044
