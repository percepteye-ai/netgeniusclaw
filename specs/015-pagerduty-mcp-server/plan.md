...
Integrate the official PagerDuty MCP server to provide incident management capabilities within NetGeniusClaw. This is a community MCP server integration following the established pattern (clone → register → skills → documentation). The server provides 70 tools for incident visibility, on-call management, service catalog, and event orchestration with write tools enabled by default.
...
**Project Type**: MCP server integration (community server + NetGeniusClaw skills)
