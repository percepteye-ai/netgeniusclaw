...
- [ ] T030 Test: create peering for NetGeniusClaw Protocol MCP (RR1 ↔ 10.255.255.1, AS 65099)
...
- **~5 ASNs** (65000 for SP, 65001/65002 for CEs, 65099 for NetGeniusClaw)
- **OSPF** on all SP core devices (P1-P4, PE1-PE3, RR1) in area 0
- **EVPN/VXLAN** BGP on DC fabric (separate from SP BGP)

Good test cases:
- `routing_get_bgp_summary(device="RR1")` — should show all iBGP clients + NetGeniusClaw eBGP peer
- `routing_create_peering(device="RR1", local_ip="10.255.255.2/30", peer_ip="10.255.255.1/30", peer_asn=65099)` — NetGeniusClaw peering
