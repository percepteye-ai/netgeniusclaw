...
| Tools | ~50+ upstream; ~20 used by NetGeniusClaw's two skills (see contracts) |
| Auth | None — target-site authentication happens via a one-time manual sign-in into a persistent Chrome profile, never via NetGeniusClaw |
...
### The `--channel` default-path gotcha (and why NetGeniusClaw pins `--executablePath` instead)
...
NetGeniusClaw never stores, requests, or transmits credentials for any target site (spec 048, FR-005). Instead, sign in once per site, directly in Chrome, using whichever pattern fits your host:
...
`--headless` is fixed at process-launch time for a given `chrome-devtools-mcp` instance — it can't be flipped mid-session on one running daemon. To give operators a genuine choice per request (rather than one fixed mode for the whole integration), NetGeniusClaw registers **two independent MCP servers** from the same package:
...
| `chrome-devtools-mcp-visible` | `--headless=false` | "Watch Mode" — a real Chrome window opens wherever NetGeniusClaw is running, and you watch it navigate/click/read live |
...
## Tool Categories Used by NetGeniusClaw
