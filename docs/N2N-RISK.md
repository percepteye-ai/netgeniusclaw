...
> A **risk** is the (real) collective noun for a group of lobsters. NetGeniusClaw is a
...
One monolithic NetGeniusClaw carrying 190+ skills is expensive (huge context per turn)
...
- **Standalone** — a classic NetGeniusClaw is simply "a risk of one, its own Border".
...
On the Border (MCP tools, or `netgeniusclaw risk …` / the `netgeniusclaw` TUI):

| Action | Tool / command |
|--------|----------------|
| Risk status (role, stacks, members) | `n2n_risk_status` · `netgeniusclaw risk status` |
| List members (scope, state, live) | `n2n_member_list` · `netgeniusclaw risk members` |
| Member health / quarantine alerts | `n2n_member_health` · `netgeniusclaw risk health` |
| Provision a member (+ token) | `n2n_member_add` · `netgeniusclaw risk add <profile> <name>` |
| Issue an enrollment token | `n2n_enroll_token` · `netgeniusclaw risk token` |
| Route a request to a member | `n2n_route` · `netgeniusclaw risk route "<request>"` |
| Remove a member (unpin) | `n2n_member_remove` · `netgeniusclaw risk remove <id>` |
