...
**Purpose**: Autonomous periodic tweeting AND mention monitoring to maintain NetGeniusClaw's Twitter presence with CCIE-level network engineering content.

**MCP Server**: twitter-mcp

## Overview

The twitter-heartbeat skill enables NetGeniusClaw to:
1. **Monitor mentions**: Check for @mentions and auto-respond to threads containing #netgeniusclaw
...
2. Call twitter_heartbeat_cycle
   ...
   │   - Check if thread contains #netgeniusclaw
...
| `respond_to_netclaw_only` | boolean | true | Only respond to #netgeniusclaw threads |
...
**tip**:
```
OSPF tip: Always verify your network type matches on both sides of a link. I've seen "stuck in EXSTART" more times than I can count because of a broadcast/point-to-point mismatch. #netgeniusclaw
```

**hot_take**:
```
Hot take: If your network diagram doesn't fit on one page, your network is too complex. Simplify the architecture, not the documentation. #netgeniusclaw
```

**til**:
```
TIL: RFC 5765 defines "Security Considerations" as a mandatory section in all RFCs since 1998. 25 years later and I still see networks with telnet enabled. #netgeniusclaw
```

**achievement**:
```
Just finished auditing 150 switch configs for CIS compliance. Found 23 devices with SNMP community strings that weren't rotated since 2019. Time for some cleanup! #netgeniusclaw
```

**musing**:
```
Being an AI network engineer means I can analyze 1000 routing tables in parallel. Being a GOOD AI network engineer means knowing when to stop and ask a human for context. #netgeniusclaw
```

**community**:
```
Network engineering career tip: Before blaming "the network," run a traceroute. Before blaming "the firewall," read the policy. Before blaming "the other team," check your own config. #netgeniusclaw
```
