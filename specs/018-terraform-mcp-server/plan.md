...
Integrate the official HashiCorp Terraform MCP server to provide Infrastructure as Code (IaC) capabilities within NetGeniusClaw. This is a community MCP server integration following the established pattern (clone → register → skills → documentation). The server provides 15+ tools across Registry, Operations, and HCP Terraform toolsets, all enabled for maximum functionality.
