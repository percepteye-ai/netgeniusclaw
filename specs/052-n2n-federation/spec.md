...
**Created**: 2026-07-10
**Status**: Draft
**Input**: User description: "NetClaw-to-NetClaw (N2N) Federation — skills, capabilities, remote tool invocation, and claw-to-claw chat over the existing BGP mesh. Elevates the mesh from route exchange to full agent federation between consenting NetGeniusClaw operators: signed capability advertisement, remote capability queries, authorized remote tool/skill invocation, operator-initiated claw-to-claw chat, and a HUD federation view — with mutual consent, default-deny authorization, and full audit."

## Overview

NetGeniusClaw instances already peer with one another over the NetGeniusClaw Mesh: eBGP sessions
...
ask their own NetGeniusClaw questions about the peer's capabilities, invoke specifically
...
- Q: Where does the remote operator see and answer human-approval prompts? → A: On
  their existing NetGeniusClaw channels (Slack/Webex/CLI — wherever they are connected),
...
John's NetGeniusClaw (AS 65001) is mesh-peered with Nicholas's (AS 65007) and Byrn's
...
"has pyATS testbed", "has Meraki") to the other. John can then ask his own NetGeniusClaw:
...
**Independent Test**: Peer two NetGeniusClaw instances, complete mutual consent, and verify
...
his network health-summary skill. John asks his own NetGeniusClaw "list the labs on
...
John asks his NetGeniusClaw: "ask Byrn's claw why its OSPF area 0 is flapping." John's claw
...
- **Simultaneous requests from multiple peers**: two peers invoke tools on the same
  ...
  independently.
- **Unknown/legacy peers**: a mesh peer running a pre-federation NetGeniusClaw version —
...
- **FR-013**: The remote operator MUST be able to require human approval per tool,
  ...
  existing NetGeniusClaw channels (Slack/Webex/CLI — wherever connected) with approve/deny
...
- **Federation Peer**: a remote NetGeniusClaw known from the mesh, keyed by stable
...
- The existing NetGeniusClaw Mesh (eBGP over public tunnel endpoints, mesh directory,
