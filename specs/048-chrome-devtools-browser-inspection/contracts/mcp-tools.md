...
> These tools are provided by the upstream `chrome-devtools-mcp` package. NetGeniusClaw registers and spawns it — it does not implement the tools. The full tool set includes ~50+ tools (navigation, input, network, performance, debugging/emulation, memory/heap-snapshot, extensions, third-party/WebMCP); only the subset relevant to this feature's four user stories is documented below. Memory/heap-snapshot and extension-management tools are out of scope for this feature.
