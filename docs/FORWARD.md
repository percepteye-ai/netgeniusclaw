...
NetGeniusClaw uses the upstream Forward MCP server to query Forward
...
For an existing NetGeniusClaw install:
...
By default, NetGeniusClaw installs the `netgeniusclaw` branch of Forward MCP. Override the
...
- CGO enabled
- Git
- Network access from the NetGeniusClaw host to the Forward API
- Forward credentials
...
same NetGeniusClaw host; upstream uses this value to partition local NQE/cache state.
...
NetGeniusClaw registers the server as `forward-mcp` in `config/openclaw.json`:
...
`spec/NQELibrary.json`. NetGeniusClaw should not maintain a separate copy of that
...
Use this path when the user asks NetGeniusClaw to recreate or draw a real or
...
6. Render the normalized node/edge list with Draw.io, Markmap, or another
   NetGeniusClaw diagram skill. Do not infer links from device names or management
   IPs.
...
Forward data can be combined with other NetGeniusClaw skills:
...
Default Forward workflows in NetGeniusClaw should be read-only. Require explicit
