# MISSION03: NetGeniusClaw Mesh — BGP Peering Between NetGeniusClaw Instances

> *Any NetGeniusClaw, anywhere in the world, can peer with any other NetGeniusClaw over BGP.*

---

## Summary

MISSION03 enables NetGeniusClaw instances to form BGP peering sessions with each other over the internet using ngrok TCP tunnels. Each NetGeniusClaw keeps its local FRR lab network and additionally peers with remote NetClaws to exchange routing intelligence across independent network domains. The result is a global mesh of AI network agents sharing real routing data at machine speed.
...
|---|---|
| Single NetGeniusClaw, local FRR lab only | Any number of NetClaws peered globally |
...
```
  ...
  Each NetGeniusClaw peers only with the Route Server.
...
Enable NetGeniusClaw Mesh peering (BGP over ngrok)? [y/N] y

  BGP listen port (default 1179): 1179

  Add a remote NetGeniusClaw peer? [y/N] y
...
```bash
# 1. Install NetGeniusClaw, enable protocol + mesh
./scripts/install.sh
...
# Share this with other NetGeniusClaw operators
```
...
NetGeniusClaw instances can make routing decisions based on what the other side is telling them through communities. Not just "here's a prefix" but "here's a prefix, and here's the context."
...
Each NetGeniusClaw has visibility into its local network. With mesh peering, every NetGeniusClaw gains visibility into every other NetGeniusClaw's network. The collective RIB becomes a distributed view of global reachability.

A NetGeniusClaw instance can answer: "Can I reach 10.200.0.0/16?" by checking if any mesh peer is advertising it. This is the same question operators ask on NOC bridge calls — except the answer comes back in milliseconds.

### Level 4: NetGeniusClaw IXP (Internet Exchange Point)

Deploy a NetGeniusClaw Route Server (RS) instance. Every other NetGeniusClaw peers only with the RS. The RS reflects routes between all members (RFC 7947). This is exactly how AMS-IX, DE-CIX, and LINX work — except the members are AI agents, not ISPs.

```
                   NetGeniusClaw Route Server (AS 65000)
                   0.tcp.ngrok.io:9999
                  /    |    |    |    \
                 /     |    |    |     \
  NetClaw-A   B     C     D     E    ...
...
- **LOCAL_PREF** — each NetGeniusClaw controls its own egress policy
...
- **Seamless L2 extension** — stretch a VLAN between two NetGeniusClaw domains
...
The mesh peers continuously. Each NetGeniusClaw monitors:
...
Based on this data, each NetGeniusClaw autonomously:
...
1. Each NetGeniusClaw registers its ngrok endpoint + AS + router-ID with a lightweight registry (GitHub Gist, simple HTTP API, or even a DNS TXT record)
2. On startup, each NetGeniusClaw queries the registry for known peers
...
# 4. Remote peer connects (from another NetGeniusClaw)
...
| MISSION03 | Active | NetGeniusClaw Mesh — BGP peering between NetGeniusClaw instances via ngrok, OPEN-based peer identification, multi-peer support, route exchange, mesh IXP model |

---

*NetGeniusClaw Mesh — Every NetGeniusClaw a router. Every router a peer. Every peer a colleague.*
