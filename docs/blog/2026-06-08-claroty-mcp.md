...
# Claroty xDome joins NetGeniusClaw — OT visibility lands as MCP #74

NetGeniusClaw's coverage has always leaned IT — Cisco fabrics, F5 load balancers, NetBox, ServiceNow, the usual suspects. Today we close one of the biggest visibility gaps on the OT side by bringing **Claroty xDome** in as MCP server #74, with three new skills and 21 tools (15 read-only, 6 ITSM-gated writes).
...
- **No `CLAROTY_LAB_MODE`.** NetGeniusClaw already has `NETCLAW_LAB_MODE` (gnmi) and `ITSM_LAB_MODE` (nautobot) in use. Introducing a third would make the lab-mode story worse. We chose `NETCLAW_LAB_MODE` because it's the one the constitution mentions and the one `itsm_gate.py` already uses.
- **Edge sensor lifecycle deferred.** The xDome API exposes 59 operations; v1 implements 21 (the highest-value reads plus the writes that map cleanly to NetGeniusClaw's CR taxonomy). Sensor lifecycle (add/update/delete locations, generate/rotate API keys) gets its own spec — it has its own CR shape and probably its own skill.
