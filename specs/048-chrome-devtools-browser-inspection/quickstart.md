...
1. Node.js 18+ on the NetGeniusClaw host
...
The single `--headless=true` flag is the only thing NetGeniusClaw pins; everything else (including the persistent profile path above) uses the tool's own defaults.
...
**Pattern A — NetGeniusClaw host has a display** (desktop Linux, or WSL2 with WSLg):
...
Tunnel that port to the NetGeniusClaw host:
```bash
ssh -R 9222:localhost:9222 <netclaw-host>
```

Sign into the target site in that local, visible Chrome window. Then, on the NetGeniusClaw host, point the MCP registration at the tunnel for that session:
```bash
...
```
@NetGeniusClaw watch it log into https://demo.netbox.dev/ and create a new site called "Test Site 1"
```
...
4. `browser-gui-inspect` against an authenticated dashboard (after completing sign-in via Pattern A or B) → expect the requested information returned without any credential prompt reaching NetGeniusClaw.
