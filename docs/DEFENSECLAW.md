...
**DefenseClaw** from Cisco AI Defense + **OpenShell** from NVIDIA provide comprehensive enterprise security for NetGeniusClaw. Together they deliver container-based isolation, runtime guardrails, and full audit compliance.
...
3. [Running NetGeniusClaw Securely](#running-netclaw-securely)
...
The NetGeniusClaw enterprise security stack combines two technologies:
...
During NetGeniusClaw installation:
...
## Running NetGeniusClaw Securely
...
3. **Ring 3**: Creates NetGeniusClaw sandbox with latest OpenClaw
...
# 2. Create a NetGeniusClaw sandbox
openshell sandbox create netgeniusclaw

# 3. Run NetGeniusClaw inside the sandbox
...
This runs NetGeniusClaw with DefenseClaw guardrails but without container isolation.
...
# Delete a sandbox
openshell sandbox delete netgeniusclaw

# View sandbox logs
openshell logs netgeniusclaw
```
...
openshell run netgeniusclaw -- ls -la

# Run NetGeniusClaw/claw
openshell run netgeniusclaw -- claw
...
name: netgeniusclaw
description: NetGeniusClaw network automation agent
...
```bash
# View current policy
openshell policy get netgeniusclaw

# Apply custom policy
openshell policy set netgeniusclaw --policy my-policy.yaml --wait
```
