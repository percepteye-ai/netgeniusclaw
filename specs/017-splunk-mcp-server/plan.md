...
Integrate the official Splunk MCP server to provide log aggregation and SPL query capabilities within NetGeniusClaw. This is a community MCP server integration following the established pattern (clone → register → skills → documentation). The server provides 7 tools with built-in SPL validation and output sanitization, with results formatted as Markdown tables.
