...
As a network engineer evaluating NetGeniusClaw, I want to see a complete list of MCP integrations so that I can determine if my infrastructure tools are supported.
...
As a prospective user, I want feature descriptions that reflect current capabilities so that I can make informed decisions about adopting NetGeniusClaw.
...
As a user navigating NetGeniusClaw documentation, I want Visual HUD references to accurately describe the current UI so that I'm not confused by outdated screenshots or descriptions.
