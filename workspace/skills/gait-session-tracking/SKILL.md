...
description: "GAIT session lifecycle management - branch creation, turn recording, audit logging for every NetGeniusClaw operation. Use when starting a new NetGeniusClaw session, recording a health check or config change, pinning a pre-change baseline, or viewing the audit trail for a troubleshooting session."
license: Apache-2.0
user-invocable: true
metadata:
...
**This skill is mandatory.** Every NetGeniusClaw session MUST begin with `gait_branch` and end with `gait_log`.
...
Creates a new GAIT repository if one does not already exist. Run this once during initial NetGeniusClaw setup.
...
Every NetGeniusClaw session follows this exact lifecycle:
...
GAIT session tracking is used by every other NetGeniusClaw skill for audit compliance:
...
Always. Every NetGeniusClaw session. No exceptions. This is the audit backbone of the system.
