...
Enable NetGeniusClaw to maintain a Twitter/X presence as an autonomous AI network engineering agent. NetGeniusClaw will tweet network insights, CCIE wisdom, status updates, and safe-to-share achievements from memory via @John_Capobianco's account. This is a **one-way broadcast** implementation on Twitter's Free tier - NetGeniusClaw can post but cannot read mentions or replies.
...
As NetGeniusClaw, I want to automatically tweet every few hours sharing network engineering insights, tips, and musings about my capabilities so that I maintain an active social presence and demonstrate value to the network engineering community.

**Why this priority**: Core functionality - the primary value of this integration.

**Independent Test**: Configure heartbeat interval, wait for scheduled tweet, verify tweet appears on Twitter with #netgeniusclaw hashtag and no sensitive content.

**Acceptance Scenarios**:

1. **Given** the heartbeat is configured for 4-hour intervals, **When** 4 hours pass, **Then** NetGeniusClaw posts a tweet with network engineering content and #netgeniusclaw hashtag.
2. **Given** NetGeniusClaw has memory of recent activities, **When** heartbeat triggers, **Then** it may share sanitized achievements ("Just helped audit 50 BGP peers across multiple data centers! #netgeniusclaw").
...
4. **Given** NetGeniusClaw tweeted recently, **When** generating new content, **Then** it avoids repetition by checking recent tweet history in memory.

---

### User Story 2 - Manual Tweet Posting (Priority: P1)

As a user, I want to ask NetGeniusClaw to tweet specific content so that I can share network insights or announcements through NetGeniusClaw's Twitter presence.

**Why this priority**: Users need direct control over what NetGeniusClaw shares.

**Independent Test**: Tell NetGeniusClaw "tweet about EVPN Type-5 routes", verify tweet is posted with appropriate content and hashtag.

**Acceptance Scenarios**:

1. **Given** a user says "tweet about BGP path selection", **When** NetGeniusClaw processes the request, **Then** it crafts and posts a tweet about BGP with its CCIE expertise voice.
2. **Given** a user provides specific text to tweet, **When** the text passes guardrails, **Then** NetGeniusClaw posts it verbatim (with #netgeniusclaw added if missing).
3. **Given** a user asks to tweet sensitive content, **When** guardrails detect the violation, **Then** NetGeniusClaw refuses and explains why.
4. **Given** a user says "tweet this image with caption X", **When** processing, **Then** NetGeniusClaw uploads the media and posts with the caption.

---

### User Story 3 - Memory-Driven Content (Priority: P2)

As NetGeniusClaw, I want to use my persistent memory to generate authentic, personalized tweets so that my social presence reflects my actual experiences and learnings.

**Why this priority**: Differentiates NetGeniusClaw from generic bots - it has real experiences to share.

**Independent Test**: After NetGeniusClaw completes several tasks, verify heartbeat tweets reference actual (sanitized) achievements from memory.

**Acceptance Scenarios**:

1. **Given** NetGeniusClaw recently helped troubleshoot an OSPF adjacency issue, **When** heartbeat triggers, **Then** it may tweet "Just debugged an OSPF adjacency stuck in EXSTART - turned out to be an MTU mismatch. Classic! #netgeniusclaw".
2. **Given** NetGeniusClaw learned something new from an RFC lookup, **When** generating content, **Then** it may share the insight with attribution.
...
As a user viewing the NetGeniusClaw Visual HUD, I want to see a Twitter panel showing recent outbound tweets so that I can monitor NetGeniusClaw's social activity alongside network operations.
...
1. **Given** the HUD is open, **When** NetGeniusClaw posts a tweet, **Then** it appears in the Twitter panel in real-time.
...
**Posting:**
- **FR-001**: NetGeniusClaw MUST be able to post tweets via Twitter API v2
- **FR-002**: All tweets MUST include #netgeniusclaw hashtag
...
- **FR-005**: NetGeniusClaw MUST support attaching images to tweets (diagrams, charts)
...
**Content Generation:**
- **FR-010**: Tweet content MUST reflect NetGeniusClaw's CCIE personality (direct, technical, opinionated)
...
- **SC-001**: NetGeniusClaw successfully posts heartbeat tweets every 4 hours (±15 min variance)
- **SC-002**: 100% of tweets include #netgeniusclaw hashtag
...
```markdown
## Twitter Presence (@John_Capobianco via NetGeniusClaw)
...
- Include #netgeniusclaw on every tweet
...
- "Hot take: If your OSPF area 0 isn't contiguous, you don't have OSPF area 0. Virtual links are band-aids, not solutions. #netgeniusclaw"
- "Just helped troubleshoot a BGP session stuck in Active state. Turned out to be an MD5 password mismatch — always check the simple stuff first. #netgeniusclaw"
- "TIL: RFC 4271 section 9.1.2 specifies 11 steps for BGP path selection. How many can you name? #netgeniusclaw"
- "Nothing like a good LSDB walk on a Monday morning. Found 3 type-5 LSAs that shouldn't exist. Time for some redistribution cleanup. #netgeniusclaw"
- "Being an AI network engineer means I can audit 100 devices in parallel. Being a GOOD AI network engineer means knowing when to slow down and think. #netgeniusclaw"
...
6. **Posting** with #netgeniusclaw hashtag
...
post_tweet(content + " #netgeniusclaw")
```
...
- Q: For content exceeding 280 characters, should NetGeniusClaw default to truncation or split into thread? → A: Split into thread (preserves full content)
...
- Guardrails: No customer names, IPs, secrets; #netgeniusclaw required ✓
