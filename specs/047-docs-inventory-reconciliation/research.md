...
**Alternatives considered**:
- *Auto-generate the README's MCP/Skills tables from `config/openclaw.json` and `workspace/skills/` metadata*: would prevent table-omission drift permanently, but requires either a templating step wired into CI or a pre-commit hook, and NetGeniusClaw's docs currently contain substantial hand-written prose per skill/server (descriptions, tool counts, env vars) that isn't mechanically derivable from directory names alone. Rejected as out of scope for this documentation-focused fix; noted as a possible future enhancement once a verifier is in place and proven useful.
