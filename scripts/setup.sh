...
if want "twilio" "Configure Twilio Voice?"; then
    ...
    echo ""
    echo -e "  ${BOLD}Model Integration (Full Agentic Mode)${NC}"
    echo "  For intelligent voice commands, point this at your model server."
    echo "  A local vLLM / LM Studio / SGLang server needs no real credential."
    echo ""
    prompt MODEL_BASE_URL "Model base URL" "http://127.0.0.1:8000/v1"
    [ -n "$MODEL_BASE_URL" ] && set_env "NETGENIUSCLAW_MODEL_BASE_URL" "$MODEL_BASE_URL"
    prompt_secret MODEL_API_KEY "Model API key [optional; blank = vllm-local]"
    set_env "NETGENIUSCLAW_MODEL_API_KEY" "${MODEL_API_KEY:-vllm-local}"
    echo ""
