...
document describes the protocol as implemented in the open-source NetGeniusClaw project
...
: Any long-running, autonomous AI agent process that implements the NCFED state
  ...
  implementation-agnostic: distinct agent architectures (for example, the NetGeniusClaw
  ...
  protocol. "NetGeniusClaw" refers specifically to the reference implementation
...
(the NetGeniusClaw reference engine and the independently developed Hermes agent) exercises
...
**Implementation:** the open-source NetGeniusClaw project (`automateyournetwork/netclaw`).
...
**Multi-implementation interoperability (Hermes <-> NetGeniusClaw).** NCFED has been
exercised between two independently developed agent implementations with different
codebases and different reasoning runtimes: the NetGeniusClaw reference engine (running a
