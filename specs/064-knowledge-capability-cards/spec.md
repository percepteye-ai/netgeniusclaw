...
**Created**: 2026-07-19
**Status**: Draft
**Input**: User description: "Adjust the A2A Agent Cards to include RAG capabilities (the knowledge base — the actual documents a NetGeniusClaw has in its local RAG) as a Skill (so it fits inside the A2A / NCFED capability card), plus instructional adjustments so routing considers remote claws' knowledge bases."
...
book) is to ask it and see. The Hermes↔NetGeniusClaw book-summary interop proved the *retrieval*
...
A NetGeniusClaw operator has documents in local RAG (feature 062). Their claw's capability card
...
Hermes↔NetGeniusClaw book demo into deliberate routing.
...
- **FR-004**: Retrieval against a peer's advertised collection MUST be exposed as a
  **dedicated NCFED method** `n2n/knowledge/query` (a member of the method families
  ...
  free-form chat path (proven in the Hermes↔NetGeniusClaw interop) MAY remain as a convenience,
...
- **SC-002**: For a knowledge question answerable only by a federated peer's corpus, the
  ...
  (no local fabrication), matching the Hermes↔NetGeniusClaw round-trip already demonstrated.
...
- The federated retrieval transport already works (proven in the Hermes↔NetGeniusClaw book-summary
