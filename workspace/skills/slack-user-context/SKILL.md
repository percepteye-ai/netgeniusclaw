...
| `users:write` | Set NetGeniusClaw's own presence status |
...
When NetGeniusClaw needs to escalate:
...
If NetGeniusClaw sees a user is already active in multiple incident threads:
...
## NetGeniusClaw Presence Management

Set NetGeniusClaw's own presence to reflect its status:

| NetGeniusClaw Status | Slack Presence | Meaning |
|----------------|---------------|---------|
