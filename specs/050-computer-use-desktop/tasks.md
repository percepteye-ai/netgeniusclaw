...
**Organization**: Tasks are grouped by user story per spec.md's priorities. This is a configuration/skill-authoring integration on top of OpenClaw's own ClawHub `computer-use` skill — no server code is authored by NetGeniusClaw.
...
## Phase 5: User Story 3 — Watch NetGeniusClaw Operate the Virtual Desktop Live (Priority: P3)

**Goal**: An operator can watch or take over the virtual desktop live via VNC/noVNC while NetGeniusClaw works, or to complete a step that needs a human.
