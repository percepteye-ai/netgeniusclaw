...
This document captures research findings for integrating the Unreal Engine 5.8 MCP server with NetGeniusClaw for 3D network topology visualization.
...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   NetGeniusClaw/WSL   │────▶│ Unreal MCP      │────▶│ UE5 Editor      │
...
- Follows existing pattern for remote/HTTP MCP servers in NetGeniusClaw
...
- Scene query via `get_all_actors_with_tag("netgeniusclaw")` to find existing topology actors
...
2. **Existing Patterns**: Follows NetGeniusClaw's URL-based MCP server registration (like Jenkins, Datadog)
