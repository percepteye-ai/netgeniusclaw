...
Implement a hybrid memory MCP server that provides persistent context across NetGeniusClaw sessions through three complementary storage layers: structured facts (SQLite with temporal validity), semantic search (ChromaDB with sentence-transformers embeddings), and entity relationship graphs. The server exposes 10 MCP tools enabling the agent to record, query, and reason about network knowledge that survives restarts.
...
**Structure Decision**: Single MCP server project with modular storage backends. Deployed via uvx (consistent with other NetGeniusClaw MCP servers). OpenShell provides container isolation at the platform level.
