...
**Goal**: Extend `browser-gui-inspect` with a network-request inspection workflow so a NetGeniusClaw contributor can identify the API endpoint(s) backing a vendor dashboard feature before writing a new API-based skill.
...
**Goal**: Extend `browser-gui-inspect` with a documented pattern for ad hoc navigation/reading/interaction against any browser-based tool with no existing NetGeniusClaw integration.

**Independent Test**: Point the skill at a web page unrelated to any existing NetGeniusClaw skill (e.g., a classic SDN controller GUI), describe what should be read or done, and confirm it performs the requested navigation/interaction and reports the resulting page state or extracted value (quickstart.md's ONOS GUI example).
...
- This is a configuration integration — no server code is authored by NetGeniusClaw; the official `chrome-devtools-mcp` package handles all tool implementation
