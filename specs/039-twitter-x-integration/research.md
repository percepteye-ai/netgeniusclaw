...
# Post a tweet
response = client.create_tweet(text="Hello from NetGeniusClaw! #netgeniusclaw")
...
```
Input (350 chars):
"BGP path selection is a 11-step process defined in RFC 4271. The first step is weight (Cisco proprietary). Then local preference. Then locally originated routes. Then AS path length. Most networks only see the first 4 steps in action. #netgeniusclaw"

Output (2 tweets):
[1/2] "BGP path selection is a 11-step process defined in RFC 4271. The first step is weight (Cisco proprietary). Then local preference. Then locally originated routes. #netgeniusclaw"
[2/2] "Then AS path length. Most networks only see the first 4 steps in action. #netgeniusclaw"
```

**Implementation Notes**:
- Reserve ~10 chars for numbering `[NN/NN] `
- Always include #netgeniusclaw in last tweet of thread
...
- Consistent with NetGeniusClaw's memory architecture
