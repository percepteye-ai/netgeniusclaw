# rag-mcp — NetGeniusClaw Agentic RAG Knowledge Base

Fully **offline, free, local** document knowledge base for NetGeniusClaw (Feature 062). Users teach NetGeniusClaw by uploading documents — vendor guides, standards, customer design docs, install guides — via Slack attachment, the HUD Knowledge panel, or URL. Retrieval is agentic: NetGeniusClaw decides when to search, critiques results, refines queries within a budget, and cites every claim.

**This is not memory.** rag-mcp stores what USERS upload (`~/.openclaw/rag/`); the Memory MCP stores NetGeniusClaw's own experience (`~/.openclaw/memory/`). Neither writes into the other.
...
- **Install**: `pip3 install -e mcp-servers/rag-mcp` (done by the NetGeniusClaw modular installer, component `rag-mcp`, default-on). Models (~250 MB total) download once at install; the system is fully offline afterwards. Air-gapped hosts pre-seed the HuggingFace cache.
