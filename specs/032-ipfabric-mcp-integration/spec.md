# Feature Specification: IP Fabric MCP Integration for NetGeniusClaw

**Feature Branch**: `032-ipfabric-mcp-integration`
**Created**: 2026-06-19
**Status**: Draft
**Input**: Production-grade integration of the official IP Fabric MCP Server into NetGeniusClaw with unified `/ipfabric` skill, developed in collaboration with Daren Fulwell (Field CTO, IP Fabric) and John Capobianco (Creator, NetGeniusClaw), representing nearly a decade of friendship and professional partnership.

## Background

IP Fabric provides an automated network assurance platform that discovers, models, and analyzes network infrastructure. The IP Fabric MCP Server is built directly into IP Fabric appliances and exposes network data through standardized MCP tools, prompts, and resources. This integration brings IP Fabric's powerful network analysis capabilities to NetGeniusClaw users through a unified `/ipfabric` skill.
...
A network security engineer wants to correlate IP Fabric network data with other NetGeniusClaw data sources to perform comprehensive analysis across multiple platforms.

**Why this priority**: Cross-platform composition differentiates NetGeniusClaw from standalone tools but requires multiple integrations to be configured. Enables advanced multi-source analysis.

**Independent Test**: Requires both IP Fabric and at least one other NetGeniusClaw skill configured; can be tested by querying "compare IP Fabric topology with my CML lab".
...
A new NetGeniusClaw user wants to enable IP Fabric integration during initial setup, or an existing user wants to add IP Fabric support to their installation.
...
2. **Given** an existing NetGeniusClaw installation, **When** user runs ipfabric-enable.sh, **Then** the system adds IP Fabric MCP configuration and reloads
...
- **FR-020**: System MUST be composable with other NetGeniusClaw skills (SuzieQ, Batfish, Check Point, CML, GNS3)
...
- **SC-009**: Cross-platform queries (IP Fabric + other NetGeniusClaw skills) complete successfully when both data sources are configured
...
- Network connectivity exists between NetGeniusClaw host and IP Fabric appliance over HTTPS
