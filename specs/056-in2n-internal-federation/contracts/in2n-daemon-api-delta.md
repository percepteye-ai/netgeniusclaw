...
Follows Nick's established TUI format (PRs #96 installer TUI, #115 `netgeniusclaw` command tree, #117 peering lifecycle) — reuse the primitives, do not invent a new prompt style.
...
|------|-----------|--------------------|
| Standalone vs risk | `tui_menu` | `Standalone NetGeniusClaw` · `Part of a risk` |
...
### `netgeniusclaw` command — Risk sub-tree (mirrors `peering_menu` / `netgeniusclaw peering`)
...
Non-interactive (mirrors `netgeniusclaw peering [status|bgp|n2n|ngrok]`), reading the daemon HTTP API via the existing `api_get`/`api_post` helpers:

| Command | Maps to |
|---------|---------|
| `netgeniusclaw risk status` | `GET /n2n/risk` (+ member summary on a Border) |
| `netgeniusclaw risk members` | `GET /n2n/members` |
| `netgeniusclaw risk add <profile\|custom> <name>` | provision + `POST /n2n/enroll/token` |
| `netgeniusclaw risk remove <member_id>` | `POST /n2n/members/remove` (confirm-gated) |
| `netgeniusclaw risk token [label]` | `POST /n2n/enroll/token` |
| `netgeniusclaw risk route "<request>"` | `POST /n2n/route` |

Standalone claws: `netgeniusclaw risk status` reports `standalone (a risk of one)`; the sub-menu entry is hidden or shows the standalone note (SC-008).
